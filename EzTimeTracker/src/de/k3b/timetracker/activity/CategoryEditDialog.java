...
public class CategoryEditDialog extends Dialog {
    ...
    private void showUsage() {
        ...
        final TimeSliceRepository timeSliceRepository = Factory.getInstance().createTimeSliceRepository(this.getContext());
        final int itemCount = timeSliceRepository.getCount(filterParam);
        if (itemCount > 0) {
            final int hours = (int) timeSliceRepository.getTotalDurationInHours(filterParam);
            final String label = String.format(this.getContext().getText(R.string.category_usage_format)
